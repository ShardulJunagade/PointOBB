...
nohup python exp/tools/result2ann_obb.py \
    ...
    --det_file "xxx/work_dir_epoch_42/test_pointobb_r50_fpn_2x_dota/pseudo_obb_result_test.json" \
    --save_ann "xxx/work_dir_epoch_42/test_pointobb_r50_fpn_2x_dota/pseudo_obb_result_test_ann.json" > "../logs/transform/result2ann_obb_$(date +%Y-%m-%d_%H-%M-%S).log" 2>&1 &


# Convert COCO format to DOTA format

nohup python tools_data_trans/test_cocorbox2dota.py \
    --json_name xxx/work_dir_epoch_42/test_pointobb_r50_fpn_2x_dota/pseudo_obb_result_test_ann.json\
