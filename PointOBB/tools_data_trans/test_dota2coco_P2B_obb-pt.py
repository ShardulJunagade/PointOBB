...
def DOTA2COCOTrain(srcpath, destfile, cls_names, dataset='dota', difficult='2'):
    ...
    with open(destfile, 'w') as f_out:
        ...
        for file in filenames:
            ...
            imagepath = os.path.join(imageparent, basename + img_ext)

            if not os.path.exists(imagepath) and dataset == 'dior':  # move testset in DIOR
            ...
            single_image = {}
            single_image['file_name'] = basename + img_ext
...
def DOTA2COCOTest(srcpath, destfile, cls_names, dataset='dota'):
    if dataset == 'dior':
        imageparent = os.path.join(srcpath, 'JPEGImages-test')
        img_ext = '.jpg'
    else:  # default is dota
        imageparent = os.path.join(srcpath, 'images')
        img_ext = '.png'
