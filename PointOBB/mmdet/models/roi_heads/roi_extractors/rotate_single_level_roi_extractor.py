...
class RotatedSingleRoIExtractor(BaseRoIExtractor):
    ...
    def forward(self, feats, rois, roi_scale_factor=None):
        ...
        if isinstance(self.roi_layers[0], ops.RiRoIAlignRotated):
            ...
            # Get the output size from the configuration directly
            # In your layer definition, the 'out_size' is specified in the config
            # Extract that from the first roi_layer
            if hasattr(self.roi_layers[0], 'output_size'):
                out_size = self.roi_layers[0].output_size
            else:
                # Fallback to getting from the layer config
                out_size = nn.modules.utils._pair(self.roi_layers[0].out_size_h)  # Assuming square output

        num_levels = len(feats)
        expand_dims = (-1, self.out_channels * out_size[0] * out_size[1])  # Updated to use out_size tuple

        if torch.onnx.is_in_onnx_export():
            # Work around to export mask-rcnn to onnx
            roi_feats = rois[:, :1].clone().detach()
            roi_feats = roi_feats.expand(*expand_dims)
            roi_feats = roi_feats.reshape(-1, self.out_channels, out_size[0], 
                                          out_size[1])  # Updated to use out_size indices
            roi_feats = roi_feats * 0
        else:
            roi_feats = feats[0].new_zeros(
                rois.size(0), self.out_channels, out_size[0], out_size[1])  # Updated to use out_size indices
